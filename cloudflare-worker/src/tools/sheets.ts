...
export function registerSheetsTools(server: McpServer, getCreds: GetCredsFunc) {
  ...
  server.tool("create_formatted_spreadsheet",
    ...
    "cell borders, frozen row 1, auto-resize columns, and optional financial number formats.",
    {
      ...
      })).optional().describe("Anthropic financial number formats (from XLSX skill) for specific columns"),
    },
    { readOnlyHint: false },
    withErrorHandler(async ({ title, sheets, theme = "blue", number_format_columns }) => {
      ...
      const created = await googleFetch(
        ...
        { properties: { title }, sheets: sheets.map((s,i) => ({ properties: { title: s.name, index: i } })) }
      ) as any;
      ...
      for (let si = 0; si < sheets.length; si++) {
        ...
        await sheetsRequest(accessToken, spreadsheetId, `/values/${encodeURIComponent(range)}?valueInputOption=USER_ENTERED`, "PUT",
          { range, majorDimension: "ROWS", values });

        // 3. Header: bold, colored bg, white text, center, wrap
        ...
        if (numDataRows > 0) {
          ...
          // 5. Alternating rows (odd data rows get alt_bg)
        ...
        // 6. Borders — all cells, light gray #D1D5DB
        const border = { style: "SOLID", width: 1, color: { red: 0.82, green: 0.84, blue: 0.87 } };
        allReqs.push({ updateBorders: {
          range: { sheetId, startRowIndex: 0, endRowIndex: totalRows, startColumnIndex: 0, endColumnIndex: numCols },
          top: border, bottom: border, left: border, right: border, innerHorizontal: border, innerVertical: border,
        }});

        // 7. Freeze header row
        allReqs.push({ updateSheetProperties: {
          properties: { sheetId, gridProperties: { frozenRowCount: 1 } },
          fields: "gridProperties.frozenRowCount",
        }});

        // 8. Auto-resize all columns
        allReqs.push({ autoResizeDimensions: {
          dimensions: { sheetId, dimension: "COLUMNS", startIndex: 0, endIndex: numCols },
        }});

        // 9. Header row minimum height
        allReqs.push({ updateDimensionProperties: {
          range: { sheetId, dimension: "ROWS", startIndex: 0, endIndex: 1 },
          properties: { pixelSize: 32 }, fields: "pixelSize",
        }});

        // 10. Financial number formats (Anthropic XLSX skill standards)
