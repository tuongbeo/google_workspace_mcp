...
export function registerSlidesTools(server: McpServer, getCreds: GetCredsFunc) {
  server.tool("get_presentation", "Get details and slide content of a Google Slides presentation.", {
    ...
    const data = await googleFetch(`https://slides.googleapis.com/v1/presentations/${presentation_id}`, accessToken) as any;
  ...
  server.tool("add_slide", "Add a new slide to a Google Slides presentation.", {
  ...
  }, async ({ presentation_id, layout = "BLANK", title, body_text }) => {
    const { accessToken } = await getCreds();
    const insertReq = { createSlide: { slideLayoutReference: { predefinedLayout: layout } } };
    const result = await googleFetch(`https://slides.googleapis.com/v1/presentations/${presentation_id}:batchUpdate`, accessToken, "POST", { requests: [insertReq] }) as any;
    ...
    if ((title || body_text) && newSlideId) {
      ...
      await googleFetch(`https://slides.googleapis.com/v1/presentations/${presentation_id}:batchUpdate`, accessToken, "POST", { requests: textReqs });
    }
  ...
  server.tool("batch_update_presentation", "Apply multiple raw batchUpdate requests to a Presentation.", {
    ...
    const { accessToken } = await getCreds();
    const result = await googleFetch(`https://slides.googleapis.com/v1/presentations/${presentation_id}:batchUpdate`, accessToken, "POST", { requests }) as any;
  ...
  server.tool("delete_slide", "Delete a slide from a presentation.", {
    ...
    const { accessToken } = await getCreds();
    await googleFetch(`https://slides.googleapis.com/v1/presentations/${presentation_id}:batchUpdate`, accessToken, "POST", {
...
export function registerTasksTools(server: McpServer, getCreds: GetCredsFunc) {
  ...
  }, async ({ tasklist_id = "@default", show_completed = false, show_hidden = false }) => {
    const { accessToken } = await getCreds();
  ...
  server.tool("get_task", "Get details of a specific task.", {
    task_id: z.string(),
    tasklist_id: z.string().optional().default("@default"),
  }, async ({ task_id, tasklist_id = "@default" }) => {
    const { accessToken } = await getCreds();
  ...
  }, async ({ title, tasklist_id = "@default", notes, due, parent }) => {
    const { accessToken } = await getCreds();
  ...
  }, async ({ task_id, tasklist_id = "@default", title, status, notes, due }) => {
    const { accessToken } = await getCreds();
  ...
  }, async ({ task_id, tasklist_id = "@default", parent, previous }) => {
    const { accessToken } = await getCreds();
